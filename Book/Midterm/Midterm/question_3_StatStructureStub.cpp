/* 
 * File:   question_3_StatStructureStub.cpp
 * Author: Matthew Madriagl (Template by Dr. Mark E. Lehr)
 * Duplicated on November 1st 2025, 9:59 pm
...
Stats *stat(const Array *array){
    ...
    // Allocates mode array
    if(nModes!=0)stats->mode->data=new int[nModes];
    stats->mode->size = nModes;
    
    //Fill the mode array
    int modeind = 0;    //Keeps track of where in the mode array we are
    for(int i = 1; i < array->size; i++) {
        if(array->data[i-1] == array->data[i]) {
            curr++;
            if(curr == max) {
                stats->mode->data[modeind] = array->data[i];
                modeind++;
                curr = 1;
            }
        } else curr = 1;
    ...
    stats->avg = static_cast<double>(total) / array->size;
